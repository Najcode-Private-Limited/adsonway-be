...
const {
   ...
   getDepositRecordsAdmin,
   getApplicationsReportAdmin,
   getRefundsReportAdmin,
   getAllUsersWithFinancialData,
} = require('../../repositories/admin_financial_report');

exports.handleGetAdminFinancialReport = asyncHandler(async (req, res) => {
   ...
   const financialReport = await getAdminFinancialReport('all', filters);
   ...
   return res
      ...
      .json(
         new ApiResponse(
            200,
            financialReport,
            'Admin financial report retrieved successfully',
            true
         )
      );
});

exports.handleExportAdminFinancialReport = asyncHandler(async (req, res) => {
   ...
   if (exportType === 'deposits' || exportType === 'all') {
      const depositRecords = await getDepositRecordsAdmin('all', filters);
   ...
   if (exportType === 'applications' || exportType === 'all') {
      const applicationsReport = await getApplicationsReportAdmin('all', filters);
   ...
   if (exportType === 'refunds' || exportType === 'all') {
      const refundsReport = await getRefundsReportAdmin('all', filters);
   ...
   res.setHeader(
      'Content-Disposition',
      `attachment; filename=admin-financial-report-${timestamp}.csv`
   );
